 ...
 */
void
write_page_to_disk(uint dev, char *pg, uint blk)   // swap out, dev(device ally ana 3awzoh), pg address of page want to write, blk referes to offset (no of block in device)
{
  //****************xv7**********************
  struct buf* buffer;
  int blockno=0;           // offset to blk
  int ithPartOfPage=0;    //which part of page (out of 8) is to be written to disk (offset l page)
  for(int i=0;i<8;i++){
    // begin_op();           //for atomicity , the block must be written to the disk
    ithPartOfPage=i*512;     // aknoh offset
    blockno=blk+i;			 // no of block ally ana feeh
 ...
 */
void
read_page_from_disk(uint dev, char *pg, uint blk)   // hna bdyloh pg(address pg ally 3awz aktb feha), dev(name of device), blk(mkan awl block fe disk)
{
  //**************xv7************************
  struct buf* buffer;
  int blockno=0;
