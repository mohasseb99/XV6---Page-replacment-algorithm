 ...
 */
void
swap_page_from_pte(pte_t *pte)  // hna low m3ana page table ally 3awzeen nsheelo
{
	//************xv7*************

  uint physicalAddress=PTE_ADDR(*pte);          //PTE_ADDR returns address in pte // physical address
 ...
 */
int
swap_page(pde_t *pgdir)  // hna low m3ana process pointer bs mosh 3arfeen anh page nshelha
{
  ...
  if(pte==0){                                     //If this is true, victim is not found in 1st attempt. Inside this function
    ...
    if(PAGEALGORITHM) clearaccessbit_LRU(pgdir);  //Accessbits are cleared,
