    ...
    if((pte=walkpgdir(pgdir,(char*)i,0))!= 0) //if mapping exists (0 as 3rd argument as we dont want to create mapping if does not exists) y3ny m3aya page table hna
		  {    // cprintf("walkpgdir successful\t");
           ...
           {   if(*pte & ~PTE_A)             //access bit is NOT set.
        	   ...
        	   // access proc -> usedCount loop to find minimum page used
        	   // if minimum page address = current page access return
                 return pte;
               }
           }
      }
...
// Clear access bit of first input page
void
clearaccessbit_FIFO(pde_t *pgdir)  // student edit
{ pte_t *pte;
  ...
  for(long i=4096;i<KERNBASE;i+=PGSIZE){
      if((pte=walkpgdir(pgdir,(char*)i,0))!= 0){
        if((*pte & PTE_P) & (*pte & PTE_A)){   //access bit is 1
        	index++;
        	if(proc -> loadOrder[index] < loadOrder){
        		loadOrder = proc -> loadOrder[index];
        		shortestPage = pte;
        	}
        }
    }
  }
  proc -> loadOrder[index] = -1;
  proc -> accessCount[index] = -1;
  *shortestPage = (*shortestPage)&(~PTE_A);
  cprintf("let's go back");
  return;
}
...
{ pte_t *pte;
  ...
  for(long i=4096;i<KERNBASE;i+=PGSIZE){
      if((pte=walkpgdir(pgdir,(char*)i,0))!= 0){
        if((*pte & PTE_P) & (*pte & PTE_A)){   //access bit is 1
        	index++;
        	if(proc -> loadOrder[index] < loadOrder && proc -> accessCount[index] <= accessCnt){
        		loadOrder = proc -> loadOrder[index];
        		accessCnt = proc -> accessCount[index];
        		LRUPage = pte;
        	}
        }
    }
